 ...
 */
$(function () {
    let arr = ["北京市", "重庆市", "福建省", "江苏省", "广东省", "辽宁省", "内蒙古", "山西省", "青海省", "四川省", "贵州省", "云南省", "陕西省", "西藏", "宁夏", "新疆", "广西", "海南省", "湖南省", "湖北省", "河南省", "山东省", "江西省", "安徽省", "浙江省", "上海市", "黑龙江省", "吉林省", "甘肃省", "天津市", "河北省"];
    let city = {
        "0": [["317", "北京市"]],
        "1": [["014", "重庆市"]],
   ...
   //渲染省市列表
    for (let i = 0; i < arr.length; i++) {
        $('.dropProvUl','.city-box').append("<li class='" + i + "'>" + arr[i] + "</li>");
    }
    //渲染城市列表并不显示
    for (let prop in city){
        $("<ul class='dropCityUl " + prop + "'></ul>")
        .hide()
        .appendTo($('.citySelector'))
        for (let j = 0; j<city[prop].length ; j++){
            $('.'+prop,'.citySelector').append("<li class='dropCityLi li" + city[prop][j][0] + "'>" + city[prop][j][1] + "</li>")
        }    
    }
    //列表绑定鼠标离开时间
    $('.city-box').on('mouseleave',function(){
        $('.city-box').hide();
    })
    $('.nav-city-box').on('click',function(){
        $('.citySelector').hide();
        $('.city-box').show();
        $('.dropProvUl').on('mouseover',function(e){
            $('.citySelector')
            .show()
            .find('.show')
            .hide()
            .removeClass('show')
            .end()
            .find('.'+e.target.className)
            .addClass('show')
            .show()
            $(e.target).on('click',function(){                
                $('.nav-city-selected').text($('.citySelector').find('.' + e.target.className).find('li').eq(0).text())
            })
            $('.citySelector')
            .find('.' + e.target.className).on('click',function(e){
                $('.nav-city-selected').text($(e.target).text());
            })
        })
    })
    //点击选择城市时，先隐藏省级市级选择块
